...
#include<string.h>	 //Core Ultra Max PW---Canny 1.0V
#include<stdio.h>	 //更新说明:写了个赞助名单 
#include<time.h>	 
...
void Homev010(string mode,string FExit,string RExit)
{
	char switch_mode;
	if(mode=="WelcomeHome")
	{
		system("title HomeWorkGo主页");
		cout<<"HomeWorkGo            一个简洁而朴实无华的写作业神器!           (Canny V1.0)"<<endl;
	  ...
	  /*cout<<"                                                                            "<<endl;*/ //比较行 
		...
		switch(switch_mode)
		{
			case 'I':Homev010("int_countF_Start","NotServer","NotServer");
					 int_countF();break;
			case 'i':Homev010("int_countF_Start","NotServer","NotServer");
					 int_countF();break;
			case 'E':exit_code();break;
			case 'e':exit_code();break;	
			case 'Z':Homev010("Zan_Start","NotServer","NotServer");
					 ZanF();break;
			case 'z':Homev010("Zan_Start","NotServer","NotServer");
					 ZanF();break;					 
		}
	...
	if(mode=="Welcome")
	{
		...
		system("cls");
		Homev010("WelcomeHome","NotSevrer","NotSevrer");
	}
...
int main()
{
	Homev010("Welcome","NotSevrer","NotSevrer");
	return 0;
}
