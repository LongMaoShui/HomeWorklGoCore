...
#include<string.h>	 //Core Ultra Max PW---Dev 0.5V
#include<stdio.h>	 //更新说明:int_countF()写完了(怎么可能) 
#include<time.h>
#include<windows.h>
using namespace std;
void int_countF()
{
	long long i,n;
	char e;
	int a,b,c,d;//a+b=c a-b=c a*b=c a/b=c......d
	cout<<"                                  整数计算                                  "<<endl;
	cout<<"输入使用次数(-1是无限循环):";
	cin>>n;
	if(n==-1)
...
void Homev05(string mode,string FExit,string RExit)
{
	char switch_mode;
	if(mode=="WelcomeHome")
	{
		system("title HomeWorkGo主页");
		cout<<"HomeWorkGo            一个简洁而朴实无华的写作业神器!             (Dev V0.5)"<<endl;
	...
	if(mode=="Welcome")
	{
		...
		Homev05("WelcomeHome","NotSevrer","NotSevrer");
	}
	if(mode=="FJiaZai")
	{
...
int main()
{
	Homev05("Welcome","NotSevrer","NotSevrer");
	return 0;
}
