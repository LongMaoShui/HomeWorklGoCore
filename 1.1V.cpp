...
#include<string.h>	 //Core Ultra Max PW---Canny 1.1V
#include<stdio.h>	 //更新说明:写了个双精度小数计算函数 
#include<time.h>	 
...
void Homev011(string mode,string FExit,string RExit)
{
	char switch_mode;
	if(mode=="WelcomeHome")
	{
		system("title HomeWorkGo主页");
		cout<<"HomeWorkGo            一个简洁而朴实无华的写作业神器!           (Canny V1.1)"<<endl;
		cout<<"                        海内存知己，天涯若比邻......                        "<<endl;
		cout<<"                          由两位小学生制作的神器!                           "<<endl;
		cout<<"                 Core Ultra Max PW尊贵版(已连接Internet版)                  "<<endl;
		cout<<"Release-整数计算(I)  Dev-双精度小数计算(D)  Bate-赞助名单(Z) Release-退出(E)"<<endl;
	  /*cout<<"                                                                            "<<endl;*/ //比较行 
		cout<<"输入:";
		cin>>switch_mode;
		switch(switch_mode)
		{
			case 'I':Homev011("int_countF_Start","NotServer","NotServer");
					 int_countF();break;
			case 'i':Homev011("int_countF_Start","NotServer","NotServer");
					 int_countF();break;
			case 'E':exit_code();break;
			case 'e':exit_code();break;	
			case 'Z':Homev011("Zan_Start","NotServer","NotServer");
					 ZanF();break;
			case 'z':Homev011("Zan_Start","NotServer","NotServer");
					 ZanF();break;	
			case 'D':Homev011("double_countF_Start","NotServer","NotServer");
					 double_countF();break;
			case 'd':Homev011("double_countF_Start","NotServer","NotServer");
					 double_countF();break;				 
		}
	...
	if(mode=="Welcome")
	{
		...
		system("cls");
		Homev011("WelcomeHome","NotSevrer","NotSevrer");
	}
...
int main()
{
	Homev011("Welcome","NotSevrer","NotSevrer");
	return 0;
}
